<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Loginn.aspx.cs" Inherits="Riyadh_Al_Salehin.Loginn" %>
...
<head>
    ...
    <title>تسجيل الدخول - رياض الصالحين (Animated)</title>
    <style>
        html, body { width:100%; height:100%; margin:0; padding:0; }
        body {
            background-color: #eff3f4; position: relative; width: 100%; height: 100%;
            font-size: 16px; font-family: 'Source Sans Pro', 'Segoe UI', Tahoma, sans-serif; font-weight: 400;
            -webkit-font-smoothing: antialiased;
        }
        form.login2 {
            position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%);
            display: block; width: 100%; max-width: 420px; background-color: #FFF;
            margin: 0; padding: 2.25em; box-sizing: border-box; border: solid 1px #DDD; border-radius: .5em;
        }
        .svgContainer { position: relative; width: 200px; height: 200px; margin: 0 auto 1em; border-radius: 50%; background: none; border: solid 2.5px #3A5E77; overflow: hidden; pointer-events: none; }
        .svgContainer > div { position: relative; width: 100%; height: 0; overflow: hidden; padding-bottom: 100%; }
        .mySVG { position: absolute; left: 0; top: 0; width: 100%; height: 100%; pointer-events: none; }

        .inputGroup { margin: 0 0 2em; padding: 0; position: relative; }
        .inputGroup:last-of-type { margin-bottom: 0; }
        label { margin: 0 0 12px; display: block; font-size: 1.1rem; color: #217093; font-weight: 700; }
        input[type='text'], input[type='password'] {
            display: block; margin: 0; padding: 14px 1em; background-color: #f3fafd; border: solid 2px #217093; border-radius: 4px; box-sizing: border-box;
            width: 100%; height: 56px; font-size: 1rem; color: #353538; font-weight: 600;
            transition: box-shadow .2s linear, border-color .25s ease-out;
        }
        input[type='text']:focus, input[type='password']:focus { outline: none; box-shadow: 0px 2px 10px rgba(0,0,0,.1); border-color: #4eb8dd; }

        .helper { position: absolute; z-index: 1; font-family: inherit; }
        .helper1 { top: 0; left: 0; transform: translate(1.4em, 2.6em) scale(1); transform-origin: 0 0; color: #217093; font-size: 1.0rem; font-weight: 400; opacity: .65; pointer-events: none; transition: transform .2s ease-out, opacity .2s linear; }
        .focusWithText .helper { transform: translate(1.4em, 2.0em) scale(.75); opacity: 1; }

        button#loginBtn {
            display: block; margin: 0; padding: .65em 1em; background-color: #4eb8dd; border: none; border-radius: 4px; width: 100%; height: 56px; font-size: 1rem; color: #FFF; font-weight: 600; cursor: pointer;
            transition: background-color .2s ease-out;
        }
        button#loginBtn:hover, button#loginBtn:active { background-color: #217093; }

        /* Responsive tweaks */
        @media (max-width: 480px) {
            .svgContainer { width: 150px; height: 150px; }
            form.login2 { padding: 1.25em; }
            input[type='text'], input[type='password'] { height: 50px; }
            button#loginBtn { height: 50px; }
        }
    </style>
</head>
<body>
    <form id="form1" runat="server" class="login2" dir="ltr">
     ...
     class="email"
     placeholder="أدخل اسم المستخدم" maxlength="256" />
    ...
    <!-- GSAP TweenMax (v2) for compatibility with the original code -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/2.1.3/TweenMax.min.js"></script>
    <script>
// Defensive, DOMContentLoaded-safe animation initialiser
document.addEventListener('DOMContentLoaded', function () {
    if (typeof TweenMax === 'undefined') {
        try { console.warn('TweenMax not found — animations disabled'); } catch (e) { }
        return;
    }

    // Query elements after DOM ready
    ...
    var dFromC, eyeDistH = 0, mouthStatus = 'small';
    ...
    function setMouthState(state) {
        ...
        if (state === 'small' && mouthSmallBG) mouthSmallBG.style.display = '';
        if (state === 'medium' && mouthMediumBG) mouthMediumBG.style.display = '';
        if (state === 'large' && mouthLargeBG) mouthLargeBG.style.display = '';
    }
    ...
    function getCoord(e) {
        ...
        var div, span;
        try {
            var carPos = email.selectionEnd || 0;
            div = document.createElement('div');
            span = document.createElement('span');
            var copyStyle = getComputedStyle(email);
            [].forEach.call(copyStyle, function (prop) { try { div.style[prop] = copyStyle[prop]; } catch (ex) { } });
            div.style.position = 'absolute'; div.style.visibility = 'hidden';
            document.body.appendChild(div);
            div.textContent = email.value.substr(0, carPos);
            span.textContent = email.value.substr(carPos) || '.'; div.appendChild(span);
            ...
            var targetX = emailCoords.x + caretCoords.x;
            ...
            var chinX = mouthX * .8, chinY = mouthY * .5;
            var chinS = 1 - ((dFromC * .15) / 100);
            if (chinS > 1) chinS = 1 - (chinS - 1);
            var faceX = mouthX * .3, faceY = mouthY * .4;
            var faceSkew = Math.cos(mouthAngle) * 5;
            var eyebrowSkew = Math.cos(mouthAngle) * 25;
            var outerEarX = Math.cos(mouthAngle) * 4, outerEarY = Math.cos(mouthAngle) * 5;
            var hairX = Math.cos(mouthAngle) * 6, hairS = 1.2;

            if (eyeL) TweenMax.to(eyeL, 1, { x: -eyeLX, y: -eyeLY, ease: Expo.easeOut });
            if (eyeR) TweenMax.to(eyeR, 1, { x: -eyeRX, y: -eyeRY, ease: Expo.easeOut });
            if (nose) TweenMax.to(nose, 1, { x: -noseX, y: -noseY, rotation: mouthR, transformOrigin: 'center center', ease: Expo.easeOut });
            if (mouth) TweenMax.to(mouth, 1, { x: -mouthX, y: -mouthY, rotation: mouthR, transformOrigin: 'center center', ease: Expo.easeOut });
            if (chin) TweenMax.to(chin, 1, { x: -chinX, y: -chinY, scaleY: chinS, ease: Expo.easeOut });
            if (face) TweenMax.to(face, 1, { x: -faceX, y: -faceY, skewX: -faceSkew, transformOrigin: 'center top', ease: Expo.easeOut });
            if (eyebrow) TweenMax.to(eyebrow, 1, { x: -faceX, y: -faceY, skewX: -eyebrowSkew, transformOrigin: 'center top', ease: Expo.easeOut });
            if (outerEarL) TweenMax.to(outerEarL, 1, { x: outerEarX, y: -outerEarY, ease: Expo.easeOut });
            if (outerEarR) TweenMax.to(outerEarR, 1, { x: outerEarX, y: outerEarY, ease: Expo.easeOut });
            if (earHairL) TweenMax.to(earHairL, 1, { x: -outerEarX, y: -outerEarY, ease: Expo.easeOut });
            if (earHairR) TweenMax.to(earHairR, 1, { x: -outerEarX, y: outerEarY, ease: Expo.easeOut });
            if (hair) TweenMax.to(hair, 1, { x: hairX, scaleY: hairS, transformOrigin: 'center bottom', ease: Expo.easeOut });
        } catch (err) {
            try { if (div && div.parentNode) div.parentNode.removeChild(div); } catch (e) { }
        } finally {
            try { if (div && div.parentNode) document.body.removeChild(div); } catch (e) { }
        }
    }
    ...
    function onEmailInput(e) {
        if (!email) return;
        getCoord(e);
        var value = (e && e.target && e.target.value) ? e.target.value : '';
        curEmailIndex = value.length;
        if (curEmailIndex > 0) {
            if (mouthStatus === 'small') {
                mouthStatus = 'medium'; setMouthState('medium');
                if (tooth) TweenMax.to(tooth, 1, { x: 0, y: 0, ease: Expo.easeOut });
                if (tongue) TweenMax.to(tongue, 1, { x: 0, y: 1, ease: Expo.easeOut });
                if (eyeL && eyeR) TweenMax.to([eyeL, eyeR], 1, { scaleX: .85, scaleY: .85, ease: Expo.easeOut });
            }
            if (value.indexOf('@') !== -1) {
                mouthStatus = 'large'; setMouthState('large');
                if (tooth) TweenMax.to(tooth, 1, { x: 3, y: -2, ease: Expo.easeOut });
                if (tongue) TweenMax.to(tongue, 1, { y: 2, ease: Expo.easeOut });
                if (eyeL && eyeR) TweenMax.to([eyeL, eyeR], 1, { scaleX: .65, scaleY: .65, ease: Expo.easeOut, transformOrigin: 'center center' });
            } else {
                mouthStatus = 'medium'; setMouthState('medium');
                if (tooth) TweenMax.to(tooth, 1, { x: 0, y: 0, ease: Expo.easeOut });
                if (tongue) TweenMax.to(tongue, 1, { x: 0, y: 1, ease: Expo.easeOut });
            ...
            mouthStatus = 'small'; setMouthState('small');
            if (tooth) TweenMax.to(tooth, 1, { x: 0, y: 0, ease: Expo.easeOut });
            if (tongue) TweenMax.to(tongue, 1, { y: 0, ease: Expo.easeOut });
            if (eyeL && eyeR) TweenMax.to([eyeL, eyeR], 1, { scaleX: 1, scaleY: 1, ease: Expo.easeOut });
        }
    }

    function onEmailFocus(e) { if (e && e.target && e.target.parentElement) e.target.parentElement.classList.add('focusWithText'); getCoord(e); }
    function onEmailBlur(e) { if (e && e.target && e.target.value === '') { if (e.target.parentElement) e.target.parentElement.classList.remove('focusWithText'); } resetFace(); }
    function onPasswordFocus(e) { coverEyes(); }
    function onPasswordBlur(e) { uncoverEyes(); }

    function coverEyes() { if (armL) TweenMax.to(armL, .45, { x: -93, y: 2, rotation: 0, ease: Quad.easeOut }); if (armR) TweenMax.to(armR, .45, { x: -93, y: 2, rotation: 0, ease: Quad.easeOut, delay: .1 }); }
    function uncoverEyes() { if (armL) TweenMax.to(armL, 1.35, { y: 220, ease: Quad.easeOut }); if (armL) TweenMax.to(armL, 1.35, { rotation: 105, ease: Quad.easeOut, delay: .1 }); if (armR) TweenMax.to(armR, 1.35, { y: 220, ease: Quad.easeOut }); if (armR) TweenMax.to(armR, 1.35, { rotation: -105, ease: Quad.easeOut, delay: .1 }); }
    function resetFace() { if (eyeL && eyeR) TweenMax.to([eyeL, eyeR], 1, { x: 0, y: 0, ease: Expo.easeOut }); if (nose) TweenMax.to(nose, 1, { x: 0, y: 0, scaleX: 1, scaleY: 1, ease: Expo.easeOut }); if (mouth) TweenMax.to(mouth, 1, { x: 0, y: 0, rotation: 0, ease: Expo.easeOut }); if (chin) TweenMax.to(chin, 1, { x: 0, y: 0, scaleY: 1, ease: Expo.easeOut }); if (face && eyebrow) TweenMax.to([face, eyebrow], 1, { x: 0, y: 0, skewX: 0, ease: Expo.easeOut }); if (outerEarL && outerEarR && earHairL && earHairR && hair) TweenMax.to([outerEarL, outerEarR, earHairL, earHairR, hair], 1, { x: 0, y: 0, scaleY: 1, ease: Expo.easeOut }); }

    if (email) { email.addEventListener('focus', onEmailFocus); email.addEventListener('blur', onEmailBlur); email.addEventListener('input', onEmailInput); }
    if (password) { password.addEventListener('focus', onPasswordFocus); password.addEventListener('blur', onPasswordBlur); }

    setMouthState('small');
    if (typeof TweenMax !== 'undefined') { if (armL) TweenMax.set(armL, { x: -93, y: 220, rotation: 105, transformOrigin: 'top left' }); if (armR) TweenMax.set(armR, { x: -93, y: 220, rotation: -105, transformOrigin: 'top right' }); }
});
</script>
</body>
</html>